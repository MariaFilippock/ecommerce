...
export const postNewProductThunk = (product: IProductType, onSuccessAdd: () => void): AppThunk => async (dispatch) => {
    try {
        ...
        const res = await fetch(`${API_HOST}/api/products/create`, {
            method: 'POST',
        ...
        const productsRes = await fetch(`${API_HOST}/api/products`);
...
export const editProductThunk = (product: IProductType): AppThunk => async (dispatch) => {
    try {
        const res = await fetch(`${API_HOST}/api/products/update`, {
            method: 'PUT',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify(product)
        })
